...
Long-lived Node processes started through the npm scripts use `paperclip-battler-*` process titles. The MCP server also sets `process.title`, and attract-mode browser profiles are created under temp folders named like `paperclip-battler-attract-room-abc123-player-left-*` so Edge/Chrome subprocesses are recognizable in process views. Each attract browser is owned by a `paperclip-battler-attract-watchdog` process that shuts down the browser tree and removes its temp profile when the MCP server stops or disappears. Set `PAPERCLIP_PROCESS_TITLE` or `PAPERCLIP_ATTRACT_PROCESS_LABEL` to customize those labels.
